...
- **Script source order.** If you've ever wondered why there's no dependency resolution for dotfiles scripts — you're not alone. Most setups use numbered prefixes (`00-base.zsh`, `10-path.zsh`, `20-tools.zsh`) or just hope everything sources in the right order. That breaks down when a tool config script needs the PATH that a base script sets up, which depends on an env script that needs to run first. dotr lets scripts declare `@after` dependencies on each other; the load order in `init.sh` is resolved automatically.
