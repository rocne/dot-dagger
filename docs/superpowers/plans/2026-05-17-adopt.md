...
var _ = strings.HasPrefix // used by Infer (Task 3 Step 4)
var _ = filepath.Join     // used by Infer (Task 3 Step 4)
...
Expected: most Infer tests FAIL because `Infer` returns `Inference{Unknown: true}` always. `TestInfer_Unknown` will PASS (the stub matches its expectation — that's fine).
...
Remove `var _ = strings.HasPrefix` and `var _ = filepath.Join` — now used by Infer. Keep `var _ = node.DeriveName` and `var _ = io.Copy` — still needed (Adopt not yet implemented).
...
Remove the remaining `var _ = node.DeriveName` and `var _ = io.Copy` suppression lines — both are now used.
...
- [ ] **Step 4: Run test to verify it passes, tidy deps, and build succeeds**
...
Expected: `TestResolveToFlag` PASS, build clean. `go mod tidy` promotes `x/term` from indirect to direct dep in `go.mod`.
...
```bash
git add cmd/dotd/adopt.go cmd/dotd/main_test.go go.mod go.sum
